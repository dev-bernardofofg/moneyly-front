...
export const UpsertTransactionRecurringSchema = z
  .object({
    type: z.enum(["income", "expense"], { message: "Tipo inválido" }),
    title: z.string().min(1, "Título é obrigatório"),
    amount: z.string().min(1, "Valor é obrigatório").refine((v) => {
      const num = parseFloat(v.replace(/\./g, "").replace(",", "."));
      return !isNaN(num) && num > 0;
    }, "Valor deve ser maior que zero"),
    ...
    description: z.string().optional(),
    totalInstallments: z.string().optional().refine(
  ...
  .superRefine((data, ctx) => {
    if (data.frequency === "weekly" && !data.dayOfWeek) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Dia da semana é obrigatório", path: ["dayOfWeek"] });
    }
    if (data.frequency === "monthly" && !data.dayOfMonth) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Dia do mês é obrigatório", path: ["dayOfMonth"] });
    }
  });
...
export type UpsertTransactionRecurringFormValues = z.infer<
  typeof UpsertTransactionRecurringSchema
>;