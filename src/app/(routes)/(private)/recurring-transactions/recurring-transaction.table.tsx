...
import { Info, PencilIcon, PowerIcon, PowerOff, Trash2, TrendingDown, TrendingUp } from "lucide-react";
    ...
    <BaseTable
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <ConfirmActionForm
              ...
              onConfirm={() => item.isActive ? deactivate({ id: item.id }) : activate({ id: item.id })}
              title={item.isActive ? "Desativar recorrente" : "Ativar recorrente"}
              description={item.isActive ? "Desativar esta transação recorrente? Nenhuma nova execução será gerada." : "Ativar esta transação recorrente? Nova execução será gerada."}
              trigger={
      ...
      columns={[
        ...
        },
        {
          ...
          cell: (_, item) => (
            <span className="text-sm font-medium text-muted-foreground">
              {FN_UTILS_NUMBERS.formatInstallments(item.executedInstallments, item.totalInstallments)}
            </span>
          ),
        },
        {
